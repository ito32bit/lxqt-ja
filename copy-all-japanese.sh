...
mkdir -m a=rwx $PWD/rsync-backup
if [ $? -gt 0 ]; then
	echo "ディレクトリが 既に存在する時は ディレクトリを リネームするか削除して下さい。"
    exit 1
fi

echo $'\n'"# applications/"
rsync $op applications/        /usr/share/applications
echo $'\n'"# desktop-directories/"
rsync $op desktop-directories/ /usr/share/desktop-directories

# セッションの設定画面の[LXQtモジュール]と[自動起動]のアプリの名称
echo $'\n'"# autostart/"
rsync $op autostart/ /etc/xdg/autostart

# パネルのプラグイン名
echo $'\n'"# lxqt-panel/"
rsync $op lxqt-panel/ $ud/lxqt/lxqt-panel
